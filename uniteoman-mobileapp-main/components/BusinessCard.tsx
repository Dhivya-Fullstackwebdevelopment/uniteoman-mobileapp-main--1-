...
export default function BusinessCard({ business, onPress, onBook, horizontal = false }: Props) {
  const C = Colors;
  const coverUri = buildImageUrl(business.cover_image_url);
      ...
      >
        ...
        <View style={styles.hInfo}>
          <Text style={[styles.categoryLabel, { color: C.textMuted }]}>
            {business.category?.name_en?.toUpperCase()}
          </Text>
          <Text style={[styles.hName, { color: C.text }]} numberOfLines={1}>
            {business.name_en}
          </Text>
          ...
          <View style={styles.locationRow}>
            <Ionicons name="location" size={12} color="#E5484D" />
            <Text style={[styles.locationText, { color: C.textMuted }]} numberOfLines={1}>
              {business.governorate?.name_en}
            </Text>
          </View>
  ...
  // ─── Vertical card — matches uploaded screenshot ──────────
  return (
    <TouchableOpacity
      style={[styles.vCard, { backgroundColor: C.card }]}
      onPress={onPress}
    ...
    >
      ...
      <View style={styles.vBody}>
        ...
        {/* Business name */}
        <Text style={[styles.vName, { color: C.text }]} numberOfLines={2}>
          {business.name_en}
        </Text>

        {/* Stars + rating + count */}
        <View style={styles.ratingRow}>
          <StarRating rating={business.rating_avg || 0} size={14} />
          <Text style={[styles.ratingAvg, { color: C.text }]}>
            {(business.rating_avg || 0).toFixed(1)}
          </Text>
          <Text style={[styles.ratingCount, { color: C.textMuted }]}>
        ...
        {/* Location */}
        <View style={styles.locationRow}>
          <Ionicons name="location" size={13} color="#E5484D" />
          <Text style={[styles.locationText, { color: C.textMuted }]} numberOfLines={1}>
            {business.governorate?.name_en}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  hInfo: { padding: 12 },
  hName: { fontSize: 15, fontWeight: '700', marginBottom: 6, letterSpacing: -0.1 },
  ...
  // ─── Verified badge (white pill, green text) ───────────────
  verifiedBadge: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
